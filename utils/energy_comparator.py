...
# ============================================================
# UNIVERSAL ENERGY FORMULA  (kWh)
# ============================================================

def energy_leg_kwh(d_km: float, load_before: float) -> float:
    """
    Universal EVRP energy formula (kWh):
       E (kWh) = 0.436 * d_km + 0.002 * load_before
    SAME formula for OR-Tools and GA.
    """
    return 0.436 * d_km + 0.002 * load_before
...
def compute_route_energy(route, D, loads, depot=0):
    ...
    Returns a RouteEnergyReport object in kWh.
    """
    if not route:
        return RouteEnergyReport(0, [], [], 0.0)
    ...
    for node in route:
        d = float(D[prev, node])
        e = energy_leg_kwh(d, current_load)
        total_kwh += e
...
def compute_fleet_energy(routes, D, loads):
    """
    Compute energy for all vehicles in kWh.
    """
    reports = []
    total = 0.0
...
def compare_ortools_vs_ga(ortools_routes, ga_routes, data):
    ...
    # OR-Tools fleet
    ort_reports, ort_total_kwh = compute_fleet_energy(
        ortools_routes, D, loads
    )

    # GA fleet
    ga_reports, ga_total_kwh = compute_fleet_energy(
        ga_routes, D, loads
    )
...
def format_route_report(report: RouteEnergyReport):
    ...
    lines.append("From→To | Dist(km) | Load | Energy(kWh) | Cumulative(kWh)")
    lines.append("-" * 65)
    ...
    for L in report.legs:
        lines.append(
            ...
            f"{L.energy_kwh:11.3f} | "
            f"{L.cumulative_kwh:15.3f}"
        )
